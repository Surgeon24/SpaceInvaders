...
public class MenuBar extends Instance {
    ...
    public void draw(TextGraphics s, Integer score, Integer level){
        ...
        s.putString(new TerminalPosition(getX()+4, getY()), "LIVES: " + "--");
        s.putString(new TerminalPosition(Globals.width/2-7, getY()), "~('W')~ LEVEL " + level+1);
        s.putString(new TerminalPosition(Globals.width-10, getY()), "SCORE: " + score);
    }
}
